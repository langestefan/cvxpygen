...
from cvxpygen import mpqp, utils
...
def generate_code(problem, code_dir="cpg_code", solver=None, solver_opts=None,
                  enable_settings=[], unroll=False, prefix="", wrapper=True, gradient=False, cmake_args=None):
    """Generate C code to solve a CVXPY problem
    
    Args:
        cmake_args: List of additional cmake arguments to pass during build (e.g., ['-DUSE_LADEL=ON'])
    ...
    configuration = get_configuration(code_dir, solver, unroll, prefix, gradient, gradient_two_stage, explicit, cmake_args)
...
def write_c_code(problem: cp.Problem, configuration: Configuration,
    ...
    write_file(os.path.join(include_dir, "cpg_workspace.h"), "w",
    ...
    write_file(os.path.join(src_dir, "cpg_workspace.c"), "w",
                utils.write_workspace_def,
                configuration, prim_variable_info, dual_variable_info,
                parameter_info, parameter_canon, solver_interface, True)
    write_file(os.path.join(include_dir, "cpg_solve.h"), "w",
                utils.write_solve_prot,
                configuration, prim_variable_info, dual_variable_info,
                parameter_info, parameter_canon, solver_interface, parameter_canon_gradient)

    write_file(os.path.join(src_dir, "cpg_solve.c"), "w",
    ...
    if configuration.gradient:
        if configuration.gradient_two_stage:
            ...
            write_file(os.path.join(include_dir, "cpg_gradient.h"), "w",
                    gradient_interface.write_gradient_prot,
                    configuration, canon_first.prim_variable_info, canon_first.dual_variable_info,
            ...
            write_file(os.path.join(src_dir, "cpg_gradient.c"), "w",
                    gradient_interface.write_gradient_def,
                    configuration, canon_first.prim_variable_info, canon_first.dual_variable_info,
                    canon_second.prim_variable_info, canon_second.dual_variable_info,
                    canon_first.parameter_info, canon_first.parameter_canon, solver_interface)
        else:
            write_file(os.path.join(include_dir, "cpg_gradient.h"), "w",
                    solver_interface.write_gradient_prot,
                    configuration, prim_variable_info, dual_variable_info,
                    None, None,
                    parameter_info, parameter_canon, solver_interface)

            write_file(os.path.join(src_dir, "cpg_gradient.c"), "w",
                    solver_interface.write_gradient_def,
                    configuration, prim_variable_info, dual_variable_info,
                    None, None,
                    parameter_info, parameter_canon, solver_interface)
...
def get_configuration(code_dir, solver_name, unroll, prefix, gradient, gradient_two_stage, explicit, cmake_args=None) -> Configuration:
    return Configuration(code_dir, solver_name, unroll, adjust_prefix(prefix), gradient, gradient_two_stage, explicit, cmake_args or [])
...
def get_parameter_info(p_prob) -> ParameterInfo:
    ...
    for p_name, p in zip(user_p_names, p_prob.parameters):
        ...
        # dealing with matrix
        elif p_name in user_p_name_to_sparsity:
