...
class OSQPInterface(SolverInterface):
    ...
    def write_gradient_def(self, f, configuration,
        ...
        if len(changing_matrices) > 0:
            f.write("  } else {\n")
            f.write(f'    if ({" || ".join([f"{prefix}Canon_Outdated_Grad.{p_id}" for p_id in changing_matrices])}) {{\n')
            f.write("      cpg_ldl_numeric();\n")
...
class ECOSInterface(SolverInterface):
    ...
    def generate_code(self, configuration, code_dir, solver_code_dir, cvxpygen_directory,
        ...
        with open(os.path.join(code_dir, "c", "solver_code", "CMakeLists.txt")) as f:
...
class ClarabelInterface(SolverInterface):
    ...
    def __init__(self, data, p_prob, enable_settings):
        ...
        self.parameter_update_structure = {
            ...
            "q": ParameterUpdateLogic(
                update_pending_logic = UpdatePendingLogic(["q"]),
                function_call = "{prefix}cpg_copy_q()"
            ),
            "b": ParameterUpdateLogic(
                update_pending_logic = UpdatePendingLogic(["b"]),
                function_call = "{prefix}cpg_copy_b()"
            ),
        }
    ...
    def __init__(self, data, p_prob, enable_settings):
        ...
        self.parameter_update_structure = {
            "init": ParameterUpdateLogic(
                ...
                function_call="load_data(&{prefix}qoco_custom_workspace)"
            ),
            "A": ParameterUpdateLogic(
                update_pending_logic = UpdatePendingLogic(["A"]),
...
class QOCOInterface(SolverInterface):
    ...
    def __init__(self, data, p_prob, enable_settings):
        ...
        function_call += (
            ...
            f'{"NULL" if canon_constants["p"] == 0 else "{prefix}Canon_Params.b"}, '
            f'G, {"NULL" if canon_constants["m"] == 0 else "{prefix}Canon_Params.h"}, '
            f'{canon_constants["l"]}, {canon_constants["nsoc"]}, '
            f'{"NULL" if canon_constants["nsoc"] == 0 else "(int *) &{prefix}qoco_q"}, '
            'qoco_settings)'
        )
    ...
    def generate_code(self, configuration, code_dir, solver_code_dir, cvxpygen_directory,
        ...
        # Add compile definitions if QOCO_SINGLE_PRECISION is enabled in cmake_args
        if configuration.cmake_args and any("QOCO_SINGLE_PRECISION" in arg and ("ON" in arg or "True" in arg)
                                            for arg in configuration.cmake_args):
            cmake_replacements.insert(0, ("include_directories (${cpg_include})",
        ...
        # add custom cmake_args if provided
        if configuration.cmake_args:
            cmake_args_str = ", ".join([f"'{arg}'" for arg in configuration.cmake_args])
            setup_replacements.append(
                ("cmake_args = ['-DCMAKE_POSITION_INDEPENDENT_CODE=ON', '-Wno-dev']",
                 f"cmake_args = ['-DCMAKE_POSITION_INDEPENDENT_CODE=ON', '-Wno-dev', {cmake_args_str}]")
            )
